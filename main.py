        ...
        try:
            ...
            if fin_df is not None and not fin_df.empty:
                if 'jinglirun' in fin_df.columns and 'zongguben' in fin_df.columns:
                    ...
                    if zongguben > 0:
                        # 通达信金额单位10倍放大特性修复：净利润除以10，再除以总股本
                        eps = round((jinglirun / 10) / zongguben, 3)
        except Exception:
            pass # 遇到无财务数据的个股不报错，直接给 0.0
...
def generate_dashboard(today, now_time):
    ...
    <thead><tr><th>代码</th><th>名称</th><th>最新价</th><th>乖离率%</th><th>每股收益</th></tr></thead><tbody>"""
    for s in daily.get('left', []): html += f"<tr><td>{s['code']}</td><td>{s['name']}</td><td>{s['price']:.2f}</td><td>{s['bias_val']:.2f}</td><td>{s.get('eps', 0.0):.3f}</td></tr>"
    ...
    <thead><tr><th>代码</th><th>名称</th><th>最新价</th><th>MA20角度</th><th>每股收益</th></tr></thead><tbody>"""
    for s in daily.get('right', []): html += f"<tr><td>{s['code']}</td><td>{s['name']}</td><td>{s['price']:.2f}</td><td>{s['ma20_angle']:.2f}</td><td>{s.get('eps', 0.0):.3f}</td></tr>"
    ...
    <thead><tr><th>推荐日期</th><th>代码</th><th>名称</th><th>推荐价</th><th>最新价</th><th>涨跌%</th><th>每股收益</th><th>状态</th></tr></thead><tbody>"""
    ...
    for r in left_history: 
        ...
        html += f"<tr><td>{r['date']}</td><td>{r['code']}</td><td>{r['name']}</td><td>{r['price']:.2f}</td><td>{r['latest_price']:.2f}</td><td class='{color_class}'>{pct:.2f}%</td><td>{r.get('eps', 0.0):.3f}</td><td>{status}</td></tr>"
        ...
        if old_daily.get('date') and old_daily['date'] != today:
            ...
            for c in old_daily.get('left', []):
                if not any(r['code'] == c['code'] and r['date'] == old_date for r in left_history):
                    left_history.append({'code': c['code'], 'name': c['name'], 'date': old_date, 'price': c['price'], 'latest_price': c['price'], 'eps': c.get('eps', 0.0)})
            for c in old_daily.get('right', []):
                if not any(r['code'] == c['code'] and r['date'] == old_date for r in right_history):
                    right_history.append({'code': c['code'], 'name': c['name'], 'date': old_date, 'price': c['price'], 'latest_price': c['price'], 'eps': c.get('eps', 0.0)})
            # 立即保存一次结转后的历史池
